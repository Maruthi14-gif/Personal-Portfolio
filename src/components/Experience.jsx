    ...
    <div className={`relative flex flex-col lg:flex-row ${isEven ? "lg:flex-row-reverse" : ""} justify-between items-center w-full mb-12 lg:mb-16`}>
      ...
      >
        <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-purple-950/40 flex items-center justify-center">
          {exp.icon === "sports" ? (
            <Award className="w-4 h-4 md:w-5 md:h-5 text-purple-400 opacity-80" />
          ) : (
            <img
              src={exp.icon}
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          {exp.status === "current" ? (
            <span className="flex items-center gap-1 px-2.5 py-0.5 rounded text-[10px] font-bold bg-purple-500/15 border border-purple-500/30 text-purple-400 uppercase tracking-wider">
          ...
          {/* Minimal Accent Dot */}
          <span className="text-[10px] font-semibold text-white/30 uppercase tracking-wider">Experience</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-secondary text-xs md:text-sm font-semibold mb-3">
          <span className="text-purple-400 font-bold">{exp.company_name}</span>
        ...
        {/* Achievements / Points Bullet List */}
        {exp.highlights && (
          <div className="mb-4">
            ...
            <ul className="space-y-2">
              {exp.highlights.map((highlight, idx) => (
                <li key={idx} className="flex items-start gap-2 text-xs text-gray-400 leading-normal">
        ...
        {/* Impact callout Box */}
        {exp.impact && (
          <div className="bg-purple-950/20 border border-purple-900/30 rounded-xl p-3.5 mb-4 shadow-inner">
          ...
          <div className="border-t border-white/5 pt-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {exp.technologies.map((tech, idx) => (
                <span 
                  key={idx}
                  className="text-[9px] font-bold px-2 py-0.5 rounded bg-white/5 border border-white/5 text-purple-300 hover:border-purple-500/30 transition-colors"
                >
                  #{tech}
                </span>
              ))}
...
const Experience = () => {
  ...
  return (
    <div ref={containerRef} className="relative py-16">
      ...
      <div className="absolute top-20 left-10 w-[450px] h-[450px] bg-purple-600/5 rounded-full blur-[140px] pointer-events-none z-0" />
      <div className="absolute bottom-20 right-10 w-[400px] h-[400px] bg-indigo-600/5 rounded-full blur-[140px] pointer-events-none z-0" />
      ...
      <div className="relative z-10">
        ...
        <div className="relative w-full max-w-5xl mx-auto flex flex-col mb-20">
